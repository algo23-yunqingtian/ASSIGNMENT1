# -*- coding: utf-8 -*-
"""
Created on Tue May 23 21:06:00 2023
...
try:
    ...
    sys.exit(-1)
'''
本策略选取了七个特征变量组成了滑动窗口长度为580+95天的训练集,随后训练了一个二分类(上涨/下跌)的支持向量机模型.
若没有仓位则在每个星期一的时候输入标的股票近580个交易日的特征变量进行预测,并在预测结果为上涨的时候购买标的.
若已经持有仓位则在盈利大于10%的时候止盈,在星期五损失大于2%的时候止损.
特征变量为:1.收盘价/均值2.现量/均量3.最高价/均价4.最低价/均价5.现量6.区间收益率7.区间标准差
训练数据为:IFZL.CFE沪深300期货主连,时间从2018-01-01到2021-06-30
回测时间为:2021-07-01 09:00:00到2022-10-01 09:00:00
单纯采用做多策略，不考虑期指做空
'''
def init(context):
    # 订阅IF主连的5分钟bar行情
    context.symbol = 'IFZL.CFE'
    subscribe(symbols=context.symbol, frequency='300s')
    start_date = '2018-01-01'  # SVM训练起始时间
    end_date = '2021-06-30'  # SVM训练终止时间
    # 用于记录工作日
    # 获取目标股票的daily历史行情
    ...
    x_all = []
    y_all = []
    for index in range(580, (len(days) - 5)):
        # 计算三星期共580个交易日相关数据
        start_day = days[index - 580]
        end_day = days[index]
    ...
    # 准备算法需要用到的数据
    for i in range(len(days_close) - 5-580):
        if days_close[i + 5+580] > days_close[i + 580]:
            label = 1
        else:
            label = 0
        y_all.append(label)
...
def on_bar(context, bars):
    ...
    if not position and weekday == 1:
        # 获取预测用的历史数据
        data = history_n(symbol=context.symbol, frequency='1d', end_time=today, count=580,
